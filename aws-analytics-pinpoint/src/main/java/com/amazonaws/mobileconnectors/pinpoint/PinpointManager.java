...
public class PinpointManager {
    ...
    public PinpointManager(final PinpointConfiguration config) {
        try {
            ...
            PinpointClientBuilder builder = PinpointClient.builder()
            ...
            final PinpointClient analyticsServiceClient = builder.build();
            //TODO: Java SDK has only one client
//            final AmazonPinpointClient targetingServiceClient = new AmazonPinpointClient(credentialsProvider, config.getClientConfiguration());
            if (region != null && !"us-east-1".equals(region.getName())) {
                builder.endpointOverride(URI.create(String.format("pinpoint.%s.amazonaws.com", region.getName())));
            }
            ENCODING_VALIDATOR.validate();

            this.pinpointContext = new PinpointContext(analyticsServiceClient, appContext, appId, SDK_INFO, config);
            ...
            //TODO: moved this up to where we create the client
//            if (region != null && !"us-east-1".equals(region.getName())) {
