...
final class EventSourceType {
    ...
    private static final class JourneyAttributeParser extends EventSourceAttributeParser {
        ...
        public Map<String, String> parseAttributes(Bundle bundle) {
            ...
            try {
                pinpointJson = JsonParser.parseString(pinpointPayload);
            ...
            for (Map.Entry<String, JsonElement> entry : journeyJson.getAsJsonObject().entrySet()) {
                result.put(entry.getKey(), ((JsonPrimitive) entry.getValue()).getAsString());
            }
            return result;
        }
    }
