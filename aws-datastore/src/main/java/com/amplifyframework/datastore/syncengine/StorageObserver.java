...
final class StorageObserver {
    ...
    private PendingMutation<SerializedModel> toPendingMutation(StorageItemChange<? extends Model> change) {
        switch (change.type()) {
            case CREATE:
                return PendingMutation.creation(change.patchedItem(), change.modelSchema());
            case UPDATE:
                return PendingMutation.update(change.patchedItem(), change.modelSchema(), change.predicate());
            case DELETE:
                return PendingMutation.deletion(change.patchedItem(), change.modelSchema(), change.predicate());
            default:
