...
abstract class NotificationClientBase {
    ...
    private boolean buildNotificationIcons(final int iconResId, final String imageIconUrl,
                                           ...
                                           final Object notificationBuilder) throws Exception {
        try {
            Bitmap largeIconBitmap = null;
            if (imageIconUrl != null) {
                try {
                    largeIconBitmap = new DownloadImageTask(imageIconUrl).call();
            ...
            if (iconClass != null && android.os.Build.VERSION.SDK_INT >= ANDROID_NOUGAT) {
                ...
                if (imageSmallIconUrl != null) {
                    try {
                        smallIconBitmap = new DownloadImageTask(imageSmallIconUrl).call();
                    } catch (final InterruptedException e) {
    ...
    private Notification createNotification(final int iconResId, final String title, final String contentText, final String imageUrl,
                                            final String imageIconUrl, final String imageSmallIconUrl, final PendingIntent contentIntent) throws Exception {
        ...
        try {
            ...
            if (imageUrl != null) {
                try {
                    notificationImage = new DownloadImageTask(imageUrl).call();
                } catch (final InterruptedException e) {
                    log.error("Interrupted when downloading image : " + e.getMessage(), e);
                } catch (final ExecutionException e) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    log.warn("Failed to build notification", exception);
                }
